...
real function sigmab(eg,el)
  real                ::  eg , el , fi
  real                ::  dpi    = 3.1415926535897932
  real                ::  me        =      5.11e-4      !GeV
  real                ::  alfa      =      1./137.037
  real                ::  sigmat    =      6.652e-25    ! cm^2

  if ( (2.0*el/me*((el-eg)/eg)) .le. 0.0d0 ) then 
     fi = 0.0d0 
  else 
     fi=4.*(log(2.0*el/me*((el-eg)/eg))-0.5)
     sigmab = (3.0d0/(8.0*dpi))*alfa*sigmat*((1.0+(1.0-eg/el)**2)*fi-2.0d0/3.0*(1.0-eg/el)*fi)
  endif

  if (sigmab .lt. 0.0d0 ) sigmab = 0.0d0

end function sigmab


real function calcpb(eg,el)
  real                       ::  eg, el
  real                       ::  c=3e10     

  calcpb= c*sigmab(eg,el)   
  !use to be -------------- c*eg*nel*sigmab(eg,el)
  !! the electron density has been calculated in the main program
  !! the photon energy is removed as calcpb = c*eg*(sigmab(eg,el)/eg)
  !! the last part being differential crossection

  !print *, calcpb, 'crossection'
end function calcpb
...
program Brem
  !-------------------------------------------------------------------------
  ! gfortran -O3 Brem.f90 -o Brem
  !----------  this is the old compilation option avoid it!! & USE

  ! gfortran -fdefault-real-8 -O3 Brem.f90 -o Brem

  implicit none 

  integer           :: i,j,k,l,m,n,ok,ll, ll2, shap
  real              :: dpi = 3.1415926535897932
  ...
  real ,allocatable :: dE(:),dNdE(:)
  real ,allocatable :: E_gamma(:),dNdE_gamma(:)
  real ,allocatable :: ngel(:)
  real ,allocatable :: gel(:)
  real ,allocatable :: gel2(:)
  real ,allocatable :: fortrpz(:)
  real ,allocatable :: S_Brem(:,:,:),F_Brem(:,:)
  real              :: inter 


  real ,allocatable :: integel(:) 

  real              :: sfac
  real              :: d8,bic,bsyn,bcoul,bbrem,obtot
  real              :: o75
  real              :: bfac


  real ,external    :: integra,calcpb,sigmab
  ...
  print *, 'mode                          :', mode 
  print *, 'cluster                       :', ncl 
  print *, 'Sub-halo ID                   :', nrock
  print *, 'grid must be 21, 101 or 201   :', Ngrid
 
  print *,('-',i = 1,70)

  write (cg ,'(I3.3)') Ngrid
  write (cn ,'(I3.3)') nstart
  ...
  allocate(S_Brem(Ngrid, Ngrid, Ngrid)) 
  allocate(F_Brem(Ngrid, Ngrid)) 
  allocate(integel(lsusy)) 

  sfac     = dx3    * Kpc / (4.0* dpi * dl**2.0 )
  o75      = 1.0 / 75.0 
  bfac     = 1.37e-20 * (1.+z)**4.0
  ...
  do ll = 1, 1
   ...
   !  path2 = 'BremCheck/'//'/Brem2783.'//cE_gamma//'keV'

     write(cE_gamma,'(I8.8)')   nint(1e6*E_gamma(ll))
     path2 = 'BremCheck/'//'CLUSTER'//cncl//'_'//crock//'Brem.'//cE_gamma//'keV'
     ...
     do l = 1, Ngrid
        do m = 1, Ngrid
           F_Brem(l,m)  = 0.0d0   !! this is to initializ
           do n = 1, Ngrid
              do k = 1, lsum
                 gel(k)  = ome  * dE(k)
                 gel2(k) = (gel(k))**2.0  
                 ...
                 d8      = dark(l,m,n)
                 obtot   = ( el_den(l,m,n)*d8**2.0) / (me * (bic+bsyn+bcoul+bbrem) ) ! dark is the dark matter density [g cm-3]
                 !! multiplication as in above  by  el_den(l,m,n) is only for brem code
                 fortrpz(k) = ( 0.5 * ( dE(k+1) - dE(k) ) * ( dNdE(k) +dNdE(k+1) ) ) * fact * obtot  
                 !   print*, k, gel(k), obtot, fact
              enddo
              ...
              do k   = 1, lsum
                 integel(k) = 2.0*ngel(k)*calcpb(E_gamma(ll),dE(k))
              enddo


              inter          = integra(lsum, lsusy, dE,integel) ! local emissivity
              !!------------------ calculate the emissivity-------------------
              S_Brem(l,m,n)    = sfac*inter
              !!print* ,'which is the emissivity  [#photons cm-3 s-1] ', S_Brem(l,m,n) 
              F_Brem(l,m) =   F_Brem(l,m) + S_Brem(l,m,n)

           enddo
           write(12, *) l, m, F_Brem(l,m) 
        enddo
     enddo
     ...
     print *, 'Gamma-ray individual[# cm-2 s-1]', minval(S_Brem),maxval(S_Brem) 
     print *, 'Gamma-ray l.o.s [# cm-2 s-1]', minval(F_Brem),maxval(F_Brem) 
     print *, 'Energy of Gamma-ray emission used', E_gamma(ll) , '[GeV]'
     print *, 'indices', ll
     print *, 'Sub-halo ID                   :', nrock
     print *, 'grid must be 21, 101 or 201   :', Ngrid
